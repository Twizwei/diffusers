...
def log_validation(vae, unet, adapter, args, accelerator, weight_dtype, step):
    ...
    for itr, (validation_prompt, validation_image) in enumerate(zip(validation_prompts, validation_images)):
        validation_image_copy = Image.open(validation_image).convert("RGB")
        validation_image = Image.open(validation_image).convert("RGB")
        validation_image = validation_image.resize((args.resolution, args.resolution))
        validation_image = image_transforms(validation_image)

        if args.validation_raymap_target:
            validation_raymap_target = torch.from_numpy(np.load(args.validation_raymap_target[itr]))
            validation_raymap_ref = torch.from_numpy(np.load(validation_images[itr].replace("conditioning_images", "raymaps").replace("png", "npy")))
            validation_raymaps = torch.cat([validation_raymap_ref, validation_raymap_target], dim=1)
            validation_raymaps = torch.nn.functional.interpolate(validation_raymaps, size=(args.resolution, args.resolution), mode="bilinear", align_corners=False)
            validation_image = [torch.cat([validation_image, validation_raymaps[0]], dim=0)]
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            for log in image_logs:
                ...
                formatted_images = [np.asarray(validation_image_copy)]
            ...
            for log in image_logs:
                ...
                validation_prompt = log["validation_prompt"]
                validation_image = log["validation_image"]
                formatted_images.append(wandb.Image(validation_image_copy, caption="adapter conditioning"))
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Number of channels in the adapter model.",
    )
    parser.add_argument(
        "--revision",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="raymap",
        help="The column of the dataset containing the raymap.",
    )
    parser.add_argument(
        "--conditioning_raymap_column",
        type=str,
        default="conditioning_raymap",
        help="The column of the dataset containing the conditioning raymap.",
    )
    parser.add_argument(
        "--caption_column",
        type=str,
    ...
    parser.add_argument(
        "--validation_raymap_target",
        type=str,
        default=None,
        nargs="+",
        help=(
            "A set of paths to the t2iadapter validation raymap. Provide either a matching number of `--validation_prompt`s, a"
            " a single `--validation_prompt` to be used with all `--validation_image`s, or a single"
            " `--validation_image` that will be used with all `--validation_prompt`s."
        ),
    )
...
def prepare_train_dataset(dataset, accelerator):
    ...
    # raymap_transforms = transforms.Compose(
    #     [
    #         transforms.Resize(args.resolution, interpolation=transforms.InterpolationMode.BILINEAR),
    #         transforms.CenterCrop(args.resolution),
    #         transforms.ToTensor(),
    ...
    def preprocess_train(examples):
        ...
        raymaps = [np.load(os.path.join(args.train_data_dir, example)) for example in examples[args.raymap_column]]
        raymaps = [torch.from_numpy(raymap) for raymap in raymaps]
        raymaps = torch.cat(raymaps, dim=0)

        conditioning_raymaps = [np.load(os.path.join(args.train_data_dir, example)) for example in examples[args.conditioning_raymap_column]]
        conditioning_raymaps = [torch.from_numpy(raymap) for raymap in conditioning_raymaps]
        conditioning_raymaps = torch.cat(conditioning_raymaps, dim=0)
        
        all_raymaps = torch.cat([conditioning_raymaps, raymaps], dim=1)  # ref_raymap, target_raymap
        all_raymaps = torch.nn.functional.interpolate(all_raymaps, size=(args.resolution, args.resolution), mode="bilinear", align_corners=False)
        examples["conditioning_pixel_values"] = [torch.cat([conditioning_image, raymap], dim=0) for conditioning_image, raymap in zip(conditioning_images, all_raymaps)]
